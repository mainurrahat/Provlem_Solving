...
{
    if(x==0) return 1;
    if(!(x&1)) return pow(n*n,x>>1);
    else return n*pow(n*n,(x-1)>>1);
}
ll mod(ll a,ll b)
{
    return ((a%b)+b)%b;
...
void to_int(string s)
{
    int len = s.size();
    ll ans = 0;
    for (int i = 0; i < len; i++)ans = ans * 10 + (int)s[i] - '0';
    cout<< ans<<endl;
}
void to_str(ll x)
{
...
void sieve(int n)                               /// Sieve of Eratosthenes
{
    ...
    ll mot= prime_vec.size();
    cout<<mot<<endl;
//jeita lagbe seita print kore naw
//jodi size lage size nao,
//jodi prime nmbr gula lage,prime nmbr gula naw
}
////////////////////*/////////////////////
...
void solve()
{
    ...
    ll sum1=0,sum2=0,left=0,right=n-1,ans;
    while(left<=right)
    {
        if(sum1<sum2)
        {
